...
plot(fs ~ sts) 
thetas <- fscores(model_2pl, method = "EAP")
hist(thetas)


# run the mirt 2-PL GSM model
items_red <- dplyr::select(items, Des_17, Des_21, Des_24,
                Des_26, Des_29, Des_33, Car_16,
                Car_18, Car_20, Car_23, Car_25,
                Car_28, Car_30, Car_32, 
                Car_34, Car_36)
model_2pl_edit <- mirt(items_red, 1, itemnames = c(colnames(items[-1])), 
                  itemtype = 'graded', 
                  technical = list(removeEmptyRows=TRUE, parallel=TRUE), 
                  empiricalhist = TRUE, 
                  SE = TRUE, SE.type = 'sandwich',
                  verbose = FALSE)
mods <- list(twoPL = model_2pl, twoPLRed = model_2pl_edit)
itemplot(mods, 1, type = 'RE')
itemplot(mods, 2, type = 'RE')
itemplot(mods, 3, type = 'RE')
plot(mods, type="infoSE")
