...
```{r, efa.1.2out, message=FALSE, warning=FALSE, tidy=TRUE}
...
In general, the model exhibits inadequate fit. The RMSEA value, and its associated 90% confidence intervals are above the .05 threshold, the CFI and TLI are below .90. The SRMR value falls within a model that exhiits acceptable fit with a value < .80. However, becasue the other values are not in line with an acceptable fitting model, we can not say the theoretical model fit the data. 

```{r, ICM_CFA_3.cont.sum, message=FALSE, warning=FALSE, tidy=TRUE}
...
```{r, ICM_CFA_3.cont.param, message=FALSE, warning=FALSE, tidy=TRUE}
...
```{r, ICM_CFA_3.cont.mi, message=FALSE, warning=FALSE, tidy=TRUE}
...
```{r, ICM_CFA_2.cont, message=FALSE, warning=FALSE, tidy=TRUE}
# construct the MPlus code for the model
...
```{r, ICM_CFA_2.cont.sum, message=FALSE, warning=FALSE, tidy=TRUE}
# extract model summary statistics
...
```
The table below provides the parameter estimates for the model. 
```{r, ICM_CFA_2.cont.param, message=FALSE, warning=FALSE, tidy=TRUE}
# print model parameters
...
```{r, ICM_CFA_2.cont.mi, message=FALSE, warning=FALSE, tidy=TRUE}
# print model modification indices
...
ESEM EFA 2-factor
```{r, efaSEM2.cont, message=FALSE, warning=FALSE, tidy=TRUE}
# construct the MPlus code for the model
